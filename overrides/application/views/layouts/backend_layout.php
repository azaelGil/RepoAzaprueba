...
<head>
    ...
    <title><?= vars('page_title') ?? lang('backend_section') ?> | Authaz Systems</title>

    <link rel="icon" type="image/x-icon" href="<?= asset_url('assets/img/favicon.ico') ?>">
    <link rel="icon" sizes="192x192" href="<?= asset_url('assets/img/logo.png') ?>">

    <!-- CSS core de Easy!Appointments -->
    ...
    <!-- 🌈 Color de empresa (company_color) -->
    <?php component('company_color_style', ['company_color' => setting('company_color')]); ?>

    <!-- 🎨 Override visual Authaz para el BACKEND -->
    <link rel="stylesheet" href="<?= base_url('overrides/assets/css/authaz-theme.css?v=5') ?>">

    <!-- 🎨 Navbar moderna Authaz para el BACKEND -->
    <link rel="stylesheet" href="<?= base_url('overrides/assets/css/backend_navbar_authaz.css?v=4') ?>">

    <?php
    // Color principal del tenant
    $company_color = setting('company_color') ?: '#35A768';

    if (!function_exists('authaz_lighten_color')) {
        function authaz_lighten_color($hex, $percent)
        {
            $hex = ltrim($hex, '#');
    ...
    $company_color_light = authaz_lighten_color($company_color, 14);
    ?>

    <style>
        /* Variables CSS con el color del tenant */
        body#backend {
            --authaz-brand-color: <?= $company_color ?>;
            --authaz-brand-color-light: <?= $company_color_light ?>;
        }

        /* Fondo de la barra backend con degradado dinámico por tenant */
        body#backend .navbar {
            background: linear-gradient(
                90deg,
                var(--authaz-brand-color),
                var(--authaz-brand-color-light)
            );
        }
    </style>
...
<body id="backend" class="d-flex flex-column h-100">
