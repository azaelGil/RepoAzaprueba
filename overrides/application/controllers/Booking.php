 ...
 *
 * Notice: This file used to have the booking page related code which since v1.5
 * has now moved to the Booking.php controller for improved consistency.
 *
 * @package Controllers
 */
class Booking extends EA_Controller
{
    ...
    {
        // Si no hay modo (?mode=...), muestra los dos botones (reagendar o cancelar)
        if (!isset($_GET['mode'])) {
            $this->load->view('pages/reschedule_choice', ['appointment_hash' => $appointment_hash]);
            return;
        }

        // 🟥 Cancelar cita (y eliminarla también de Google Calendar)
        if (isset($_GET['mode']) && $_GET['mode'] === 'cancel') {
            ...
            $appointment = $occurrences[0] ?? null;
            ...
            if ($appointment) {
                ...
                // 1️⃣ Elimina la cita de la base de datos
                $this->appointments_model->delete((int)$appointment['id']);

                // 2️⃣ Envía notificación de cancelación
                ...
                // 3️⃣ Elimina el evento en Google Calendar
                $this->synchronization->sync_appointment_deleted($appointment, $provider);
            }

            // 4️⃣ Mensaje visual de confirmación con color dinámico del tenant
            $brandColor = setting('company_color') ?: '#000000';

            echo '
<div style="display:flex;justify-content:center;align-items:center;min-height:100vh;background:#f4f5f7;padding:20px;">
    <div style="
        background:#ffffff;
        border-radius:18px;
        padding:45px 50px;
    ...
    ">
        ...
        <h2 style="color:#000;font-size:26px;font-weight:700;margin-bottom:15px;">
            Cita cancelada correctamente
        </h2>

        <p style="color:#444;font-size:16px;margin-bottom:35px;">
            Su cita ha sido eliminada de la agenda.
        </p>
...
';
    ...
    public function register(): void
    {
        try {
            ...
            // 🔗 Enlace público para gestionar la cita
            $appointment_link = site_url('booking/reschedule/' . $appointment['hash']);
            $appointment['appointment_link'] = $appointment_link;
     ...
     *
     * Outputs a JSON string with the unavailability dates.
     */
